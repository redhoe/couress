...
type TransactionTron struct {
	...
	TransferCoinType CoinType        `json:"TransferCoinType" gorm:"type:varchar(50);comment:转账币种类型"`
...
func (*TransactionTron) GetAllPage(db *gorm.DB, paging *Paging) ([]TransactionTron, error) {
...
type TxInfo struct {
	...
	TransferCoinType      CoinType               `json:"transfer_coin_type" gorm:"type:varchar(50);comment:转账币种类型"`
	...
	CoinInfoObj           *CoinInfo              `json:"coin" gorm:"comment:币种信息"`
	WebUrl                string                 `json:"web_url"`
}

func (t TransactionTronList) ToInfo(db *gorm.DB, webUrl, walletAddress string, currencyEngine *CurrencyExchangeRate) []TxInfo {
	return lo.Map(t, func(item TransactionTron, index int) TxInfo {
		...
		if item.TransferCoinType != CoinTypeTRX {
